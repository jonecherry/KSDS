...
def AssignTask(task,network):
    ...
    if Match(network,task) != 'notfound':
        ...
        # 从邻域中找匹配度大于等于2个体加入team
        for node in network.neighbors(theFirst):
            if network.node[node]['status']=='available' and MatchDegree(network.node[node],task)>=1:
...
def process():
    ...
    while 1:
        ...
        for dtsk in rmlist:
            ...
            if len(tms)<=3 and switchbutton ==1:
                print '团队成员数量小于4'
                for ttmm in tms:
                    print str(ttmm)+'邻近节点'
                    for tttmmm in G.neighbors(ttmm):
                        print '理想团队：'+str(max_payoff(tttmmm,G)),G.node[tttmmm]
            for tm in tms:
                G.node[tm]['end'] = day
